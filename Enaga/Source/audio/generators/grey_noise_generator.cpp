...
void GreyNoiseGenerator::Prepare(double sample_rate) {
  const auto s0 =
      juce::IIRCoefficients::makeLowShelf(sample_rate, 100.0, 0.5, 10.0);
  const auto s1 =
      juce::IIRCoefficients::makePeakFilter(sample_rate, 3500.0, 1.5, 0.71);
  const auto s2 =
      juce::IIRCoefficients::makeHighShelf(sample_rate, 10000.0, 0.5, 1.5);
...
float GreyNoiseGenerator::NextSample(std::size_t channel) noexcept {
  ...
  for (auto& f : filters_[channel]) s = f.processSingleSampleRaw(s);
