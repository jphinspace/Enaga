...
void NoiseAudioSource::SetGain(float new_gain) noexcept {
  gain_.store(juce::jlimit(0.0f, 1.0f, new_gain), std::memory_order_relaxed);
}
...
void NoiseAudioSource::prepareToPlay(int /*samples_per_block_expected*/,
                                     double new_sample_rate) {
  sample_rate_ = new_sample_rate;
  fade_current_ = fade_target_.load(std::memory_order_relaxed);
  fade_step_ =
      (sample_rate_ > 0.0)
          ? 1.0f / (kFadeDurationSeconds * static_cast<float>(sample_rate_))
          : 0.0f;

  last_cutoff_ = cutoff_.load(std::memory_order_relaxed);
  UpdateLpFilters();
  for (auto& f : lp_filters_) f.reset();
...
void NoiseAudioSource::getNextAudioBlock(
  ...
  const float effective_cutoff =
      (lfo_mode_val == LfoMode::kFilter || lfo_mode_val == LfoMode::kBoth)
  ...
  const float lfo_gain_mod =
      (lfo_mode_val == LfoMode::kVolume || lfo_mode_val == LfoMode::kBoth)
  ...
  for (int ch = 0; ch < info.buffer->getNumChannels(); ++ch) {
    ...
    const auto ch_idx = static_cast<std::size_t>(juce::jmin(ch, 1));

    for (int i = 0; i < info.numSamples; ++i) out[i] = gen->NextSample(ch_idx);
...
void NoiseAudioSource::UpdateLpFilters() {
  ...
  const double freq =
      20.0 * std::pow(1000.0, static_cast<double>(last_cutoff_) / 100.0);
  const auto coef = juce::IIRCoefficients::makeLowPass(sample_rate_, freq);
  for (auto& f : lp_filters_) f.setCoefficients(coef);
}
