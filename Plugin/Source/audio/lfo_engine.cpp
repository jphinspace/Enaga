...
float LfoEngine::Tick(int num_samples, double sample_rate) noexcept {
  ...
  const float result = static_cast<float>((1.0 + std::sin(phase_)) * 0.5);

  phase_ += static_cast<double>(rate_.load(std::memory_order_relaxed)) *
...
void LfoEngine::Reset() noexcept { phase_ = 0.0; }
