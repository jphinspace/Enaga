...
juce::StringArray MainComponent::getMenuBarNames() { return {"File"}; }
...
void MainComponent::resized() {
  ...
  mobile_menu_button_.setBounds(image_area_.getRight() - m_btn_sz - 4,
                                image_area_.getY() + 4, m_btn_sz, m_btn_sz);
#else
  mobile_menu_button_.setBounds(image_area_.getX() + 4, image_area_.getY() + 4,
                                m_btn_sz, m_btn_sz);
#endif
#endif
  ...
  const int ctrl_h = (area.getHeight() - (num_rows - 1) * kPad) / num_rows;

  // Row 1: play button + label + discrete slider
  auto row1 = area.removeFromTop(ctrl_h);
  area.removeFromTop(kPad);
  const int btn_sz = juce::jlimit(32, 52, ctrl_h - 4);
  play_button_.setBounds(
      row1.removeFromLeft(btn_sz + 4).withSizeKeepingCentre(btn_sz, btn_sz));
  row1.removeFromLeft(6);
  discrete_label_.setBounds(
      row1.removeFromLeft(64).withHeight(ctrl_h).withY(row1.getY()));
  ...
  lfo_component_.setBounds(area.removeFromTop(3 * ctrl_h + 2 * kPad));
}
...
void MainComponent::SetupPlayButton() {
  play_button_.onClick = [this] {
    if (callbacks_.on_toggle)
      callbacks_.on_toggle(play_button_.getToggleState());
  };
...
void MainComponent::SetupDiscreteSlider() {
  ...
  discrete_slider_.setTextBoxStyle(juce::Slider::TextBoxLeft, false, 56, 20);
  ...
  discrete_slider_.onValueChange = [this] {
    if (callbacks_.on_noise_type)
      callbacks_.on_noise_type(static_cast<float>(discrete_slider_.getValue()));
  };
...
void MainComponent::SetupContinuousSlider() {
  ...
  continuous_slider_.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
  ...
  continuous_slider_.onValueChange = [this] {
    SyncValueBox();
    if (callbacks_.on_filter)
      callbacks_.on_filter(static_cast<float>(continuous_slider_.getValue()));
  };
...
void MainComponent::SetupValueBox() {
  ...
  continuous_value_box_.setText(juce::String(kDefaultContinuousValue, 1),
                                false);
  continuous_value_box_.setJustification(juce::Justification::centred);
...
void MainComponent::SetupLabels() {
  ...
  discrete_label_.setJustificationType(juce::Justification::centredLeft);
  addAndMakeVisible(discrete_label_);

  continuous_label_.setText("Cutoff", juce::dontSendNotification);
  continuous_label_.setJustificationType(juce::Justification::centredLeft);
  addAndMakeVisible(continuous_label_);

  volume_label_.setText("Volume", juce::dontSendNotification);
  volume_label_.setJustificationType(juce::Justification::centredLeft);
...
void MainComponent::SetupMobileMenuButton() {
  ...
  mobile_menu_button_.setButtonText(juce::CharPointer_UTF8("\xe2\x98\xb0"));
#endif
#if JUCE_IOS || JUCE_ANDROID
...
void MainComponent::SetupVolumeSlider() {
  ...
  volume_slider_.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
  volume_slider_.setRange(0.0, 100.0);
  volume_slider_.setValue(kDefaultVolumeValue);
  volume_slider_.onValueChange = [this] {
    if (callbacks_.on_gain)
      callbacks_.on_gain(static_cast<float>(volume_slider_.getValue()) /
                         100.0f);
  };
...
void MainComponent::SetupVolumeSlider() {
  ...
  const float sys_gain =
      juce::jlimit(0.0f, 1.0f, juce::SystemAudioVolume::getGain());
...
void MainComponent::PaintImageArea(juce::Graphics& g) {
  ...
  juce::Path wave;
  const float step =
      image_area_.getWidth() / static_cast<float>(kWaveformSampleCount);
  ...
  g.setFont(
      juce::Font(juce::FontOptions{}.withHeight(24.0f).withStyle("Bold")));
  g.drawText("ENAGA", image_area_, juce::Justification::centred, false);
}
...
void MainComponent::SyncValueBox() {
  continuous_value_box_.setText(juce::String(continuous_slider_.getValue(), 1),
                                false);
}

void MainComponent::ApplyValueBox() {
  const double v = juce::jlimit(
      0.0, 100.0, continuous_value_box_.getText().getDoubleValue());
...
void MainComponent::SavePreset() {
  ...
  file_chooser_->launchAsync(flags, [this](const juce::FileChooser& fc) {
    ...
    preset.setAttribute("discreteValue", discrete_slider_.getValue());
    preset.setAttribute("continuousValue", continuous_slider_.getValue());
    preset.setAttribute("isPlaying",
                        static_cast<int>(play_button_.getToggleState()));
#if !(JUCE_IOS || JUCE_ANDROID)
    preset.setAttribute("volumeValue", volume_slider_.getValue());
#endif
...
void MainComponent::LoadPreset() {
  ...
  file_chooser_->launchAsync(flags, [this](const juce::FileChooser& fc) {
    ...
    if (const auto xml = juce::XmlDocument::parse(file)) {
      const double disc_val =
          xml->getDoubleAttribute("discreteValue", kDefaultDiscreteValue);
      const double cont_val =
          xml->getDoubleAttribute("continuousValue", kDefaultContinuousValue);
...
#if !(JUCE_IOS || JUCE_ANDROID)
      const double vol_val =
          xml->getDoubleAttribute("volumeValue", kDefaultVolumeValue);
      volume_slider_.setValue(vol_val);
#endif

      if (playing != play_button_.getToggleState())
        play_button_.setToggleState(playing, juce::sendNotification);
    }
  });
}
