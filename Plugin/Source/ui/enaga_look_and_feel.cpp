...
void EnagaLookAndFeel::drawLinearSlider(
  ...
  for (double v = lo; v <= hi + interval * 0.01; v += interval) {
    const float proportion =
        static_cast<float>((v - lo) / span);
    const int tick_x =
        x + juce::roundToInt(
                proportion * static_cast<float>(width))
        - tick_w / 2;
